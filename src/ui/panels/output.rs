...
fn render_relative_mode_ui(ui: &mut egui::Ui, config: &mut MappingConfig) {
                ...
                if response.dragged() {
                    if let Some(pointer_pos) = response.interact_pointer_pos() {
                        if t_rect.expand(20.0).contains(pointer_pos) || response.drag_started() {
                            ...
                            config.target_area.x = (config.target_area.x + delta.x / scale)
                                .clamp(min_x, max_x - config.target_area.w);
                            config.target_area.y = (config.target_area.y + delta.y / scale)
...
fn render_tablet_section(app: &TabletMapperApp, ui: &mut egui::Ui, config: &mut MappingConfig) {
            ...
            if response.dragged() {
                if let Some(pointer_pos) = response.interact_pointer_pos() {
                    ...
                    if click_rect.expand(20.0).contains(pointer_pos) || response.drag_started() {
                        let delta = response.drag_delta();
                        config.active_area.x = (config.active_area.x + delta.x / scale).clamp(
                            config.active_area.w / 2.0,
                            phys_w - config.active_area.w / 2.0,
                        );
                        config.active_area.y = (config.active_area.y + delta.y / scale).clamp(
...
pub fn render_output_panel(
    ...
    if config.mode == crate::domain::DriverMode::Relative {
        render_relative_mode_ui(ui, config);
        return;
    }

    render_display_section(app, ui, config, min_x, min_y, max_x, max_y);
    ui.add_space(20.0);
    render_tablet_section(app, ui, config);
}
